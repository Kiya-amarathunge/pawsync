...
export default function ProviderProfilePage() {
  ...
  const useLocation = () => navigator.geolocation.getCurrentPosition(position => setForm(value => ({ ...value, lat: String(position.coords.latitude), lng: String(position.coords.longitude) })), () => showToast('Location permission was not granted', 'error'));
  ...
  const save = async (event: FormEvent) => {
    ...
    try {
      const payload = {
        ...
        yearsOfExperience: form.yearsOfExperience ? Number(form.yearsOfExperience) : 0, serviceRadiusKm: Number(form.serviceRadiusKm), serviceType: form.serviceType,
  ...
  return <DashboardLayout><form onSubmit={save} style={{ display: 'grid', gap: 18 }}>
    ...
    <section className="card" style={{ padding: 22, display: 'grid', gap: 14 }}><h2 style={{ fontSize: 17 }}>Public information</h2>{user?.role === 'service_provider' && <label><span className="input-label">Business name</span><input className="input" value={form.businessName} onChange={event => setForm(value => ({ ...value, businessName: event.target.value }))} required /></label>}<label><span className="input-label">Description</span><textarea className="input" rows={4} value={form.businessDescription} onChange={event => setForm(value => ({ ...value, businessDescription: event.target.value }))} /></label><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(190px, 1fr))', gap: 12 }}><label><span className="input-label">Specialization</span><input className="input" value={form.specialization} onChange={event => setForm(value => ({ ...value, specialization: event.target.value }))} /></label><label><span className="input-label">Years of experience</span><input className="input" type="number" min="0" max="80" value={form.yearsOfExperience} onChange={event => setForm(value => ({ ...value, yearsOfExperience: event.target.value }))} /></label><label><span className="input-label">Service radius (km)</span><input className="input" type="number" min="1" max="500" value={form.serviceRadiusKm} onChange={event => setForm(value => ({ ...value, serviceRadiusKm: event.target.value }))} /></label></div><label><span className="input-label">Qualifications and credentials</span><textarea className="input" rows={3} value={form.credentials} onChange={event => setForm(value => ({ ...value, credentials: event.target.value }))} /></label></section>
    ...
    <section className="card" style={{ padding: 22, display: 'grid', gap: 12 }}><h2 style={{ fontSize: 17, display: 'flex', gap: 8 }}><MapPin size={19} /> Location</h2><input className="input" placeholder="Practice or business address" value={form.address} onChange={event => setForm(value => ({ ...value, address: event.target.value }))} /><div style={{ display: 'flex', gap: 10 }}><input className="input" type="number" step="any" placeholder="Latitude" value={form.lat} onChange={event => setForm(value => ({ ...value, lat: event.target.value }))} /><input className="input" type="number" step="any" placeholder="Longitude" value={form.lng} onChange={event => setForm(value => ({ ...value, lng: event.target.value }))} /><button type="button" className="btn btn-secondary" onClick={useLocation}><MapPin size={16} /> Use current</button></div></section>
