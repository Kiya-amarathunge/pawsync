...
export default function DisputesPage() {
  ...
  return <DashboardLayout>
    ...
    {showForm && <form onSubmit={submit} className="card" style={{ padding: 20, marginBottom: 18, display: 'grid', gap: 13 }}>
      ...
      <label className="input-group"><span className="input-label">Category</span><select className="input" value={form.category} onChange={event => setForm(value => ({ ...value, category: event.target.value }))}>{[['service_quality', 'Service quality'], ['cancellation', 'Cancellation'], ['billing', 'Appointment value question'], ['refund', 'Refund recommendation request'], ['conduct', 'User conduct'], ['other', 'Other']].map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
    ...
    {loading ? <div className="skeleton" style={{ height: 120 }} /> : disputes.length === 0 ? <div className="empty-state"><CircleAlert size={28} /><p>No disputes have been submitted.</p></div> : <div style={{ display: 'grid', gap: 12 }}>
      {disputes.map(dispute => <article key={dispute._id} className="card" style={{ padding: 18 }}>
        ...
        {dispute.resolution && <div style={{ marginTop: 13, padding: 12, background: 'var(--primary-light)', borderLeft: '3px solid var(--primary)' }}><strong style={{ fontSize: 13 }}>Administrator resolution</strong><p style={{ marginTop: 4, fontSize: 13 }}>{dispute.resolution}</p>{dispute.resolutionAction === 'refund' && <p style={{ marginTop: 4, fontSize: 13 }}>Refund approval record: Rs. {(dispute.refundAmount || 0).toLocaleString()} (no payment processed by PawSync)</p>}</div>}
      </article>)}
    </div>}
  </DashboardLayout>;
}
