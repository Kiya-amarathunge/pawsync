...
interface Provider {
  ...
  acceptanceRate: number; responseTimeMinutes: number | null; isFavorite: boolean;
}
...
export default function ProvidersPage() {
  ...
  const [filters, setFilters] = useState({ serviceType: 'all', minRating: '0', location: '', specialization: '', maxPrice: '', availableOn: '' });
  ...
  return <DashboardLayout>
    ...
    {isLoading ? <div className="grid-3">{[1, 2, 3, 4, 5, 6].map(value => <div key={value} className="skeleton" style={{ height: 255 }} />)}</div> : providers.length === 0 ? <div className="empty-state"><p>No verified providers match these filters.</p></div> : <div className="grid-3">{providers.map(provider => <article key={provider._id} className="card" style={{ padding: 20, display: 'grid', gap: 12 }}>
      ...
      {provider.location?.address && <p style={{ color: 'var(--text-secondary)', fontSize: 13, display: 'flex', gap: 6 }}><MapPin size={15} /> {provider.location.address}</p>}
