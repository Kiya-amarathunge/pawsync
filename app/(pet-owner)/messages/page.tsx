...
function MessagesContent() {
  ...
  return <DashboardLayout><div style={{ marginBottom: 20 }}><h1 className="page-title">Messages</h1><p className="page-subtitle">Real-time owner and provider communication</p></div>
    <div className="card" style={{ height: 'min(680px, calc(100vh - 180px))', display: 'grid', gridTemplateColumns: '290px minmax(0, 1fr)', overflow: 'hidden', padding: 0 }}>
      <aside style={{ borderRight: '1px solid var(--border)', minHeight: 0, overflow: 'auto' }}><div style={{ padding: 14, borderBottom: '1px solid var(--border)' }}><div style={{ display: 'flex', gap: 6 }}><input className="input" placeholder="Search message text" value={query} onChange={event => { setQuery(event.target.value); if (!event.target.value) { setSearchResults([]); setHasSearched(false); } }} onKeyDown={event => event.key === 'Enter' && void searchMessages()} /><button className="btn btn-secondary btn-sm" onClick={() => void searchMessages()} aria-label="Search"><Search size={16} /></button></div></div>{hasSearched && <div style={{ padding: 10, borderBottom: '1px solid var(--border)' }}>{searchResults.length ? searchResults.map(result => <button key={result._id} onClick={() => void loadMessages(result.otherId)} style={{ display: 'block', width: '100%', border: 0, background: 'none', padding: 7, textAlign: 'left', cursor: 'pointer', fontSize: 12 }}><strong>{result.otherName}</strong><span style={{ display: 'block', color: 'var(--text-muted)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{result.content}</span></button>) : <p style={{ padding: 7, color: 'var(--text-muted)', fontSize: 12 }}>No matching messages found.</p>}</div>}{isLoading ? <div className="skeleton" style={{ margin: 14, height: 60 }} /> : conversations.map(conversation => <button key={conversation.otherId} onClick={() => void loadMessages(conversation.otherId)} style={{ width: '100%', border: 0, borderLeft: selectedId === conversation.otherId ? '3px solid var(--primary)' : '3px solid transparent', background: selectedId === conversation.otherId ? 'var(--primary-light)' : 'transparent', padding: 14, textAlign: 'left', cursor: 'pointer' }}><div style={{ display: 'flex', justifyContent: 'space-between' }}><strong style={{ fontSize: 13 }}>{conversation.otherUser?.name || 'User'}</strong>{conversation.unreadCount > 0 && <span className="badge badge-green">{conversation.unreadCount}</span>}</div><p style={{ fontSize: 12, color: 'var(--text-muted)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{conversation.lastMessage?.content || conversation.lastMessage?.attachments?.[0]?.filename}</p></button>)}</aside>
